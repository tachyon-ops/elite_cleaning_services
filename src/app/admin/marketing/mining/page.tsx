...
const KANBAN_COLUMNS = [
  { id: "new", label: "New Leads", color: "border-blue-500/30 bg-blue-500/5 text-blue-400", dot: "bg-blue-400" },
  { id: "qualified", label: "Qualified", color: "border-indigo-500/30 bg-indigo-500/5 text-indigo-400", dot: "bg-indigo-400" },
  { id: "contacted", label: "Contacted", color: "border-amber-500/30 bg-amber-500/5 text-amber-400", dot: "bg-amber-400" },
  { id: "quoted", label: "Quoted", color: "border-purple-500/30 bg-purple-500/5 text-purple-400", dot: "bg-purple-400" },
  { id: "won", label: "Won / Client", color: "border-emerald-500/30 bg-emerald-500/5 text-emerald-400", dot: "bg-emerald-400" },
  { id: "dismissed", label: "Dismissed", color: "border-neutral-700 bg-neutral-900 text-neutral-400", dot: "bg-neutral-500" },
];
...
export default function MarketingMiningPage() {
  ...
  // Header collapse state (frees vertical height for Kanban)
  ...
  // Selected lead for Master-Detail Slide-Over Drawer
  ...
  const getPriorityBadge = (score: number) => {
    if (score >= 80) {
      return (
        <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 whitespace-nowrap">
          {score} • High
        </span>
      );
    }
    if (score >= 50) {
      return (
        <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20 whitespace-nowrap">
          {score} • Med
        </span>
      );
    }
    return (
      <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-neutral-800 text-neutral-400 border border-neutral-700 whitespace-nowrap">
        {score} • Low
      </span>
    );
  };
  ...
  return (
    <div className={`min-h-screen bg-[#0d0d0d] text-[#f2f2f2] flex flex-col ${isHeaderCollapsed ? "p-4 space-y-3" : "p-6 space-y-5"}`}>
      <div className="w-full flex flex-col flex-1 min-w-0 space-y-4">
        {/* Navigation Tabs & Header Collapse Toggle */}
        <div className="flex items-center justify-between border-b border-[#262626] pb-3 shrink-0">
          <div className="flex items-center gap-4">
            <Link
              href="/admin/marketing"
              className="text-xs font-medium text-[#a6a6a6] hover:text-[#f2f2f2] transition-colors"
            >
              Campaigns & Pamphlets
            </Link>
            <span className="text-xs font-medium text-[#d4af37] border-b-2 border-[#d4af37] pb-3 -mb-3">
              Commercial Lead Mining (SHAB)
            </span>
          </div>

          {/* Toggle Header to maximize board space */}
          <button
            onClick={() => setIsHeaderCollapsed((prev) => !prev)}
            className="flex items-center gap-1.5 text-xs text-[#a6a6a6] hover:text-[#f2f2f2] bg-[#171717] hover:bg-[#222222] border border-[#2b2b2b] px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
            title={isHeaderCollapsed ? "Expand stats & header" : "Compact header to maximize Kanban board"}
          >
            {isHeaderCollapsed ? (
              <>
                <ChevronDown className="w-3.5 h-3.5 text-[#d4af37]" />
                <span className="font-medium">Show Stats & Overview</span>
              </>
            ) : (
              <>
                <ChevronUp className="w-3.5 h-3.5 text-[#d4af37]" />
                <span className="font-medium">Compact Header</span>
              </>
            )}
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 shrink-0">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-display font-semibold !text-white tracking-tight">Marketing Mining</h1>
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-[#d4af37]/15 text-[#d4af37] border border-[#d4af37]/30 flex items-center gap-1">
                <Sparkles className="w-3 h-3" /> Swiss Register Intelligence
              </span>
            </div>

            {!isHeaderCollapsed ? (
              <p className="text-xs text-[#a6a6a6] font-body mt-1">
                Live pipeline of office relocations (HR02) and new incorporations (HR01). Drag leads across columns to advance outreach.
              </p>
            ) : (
              /* Inline Summary Stats Strip in Compact Mode */
              <div className="flex flex-wrap items-center gap-2 mt-1.5">
                <span className="text-xs text-[#888888] font-medium">Pipeline:</span>
                <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-[#171717] border border-[#2b2b2b] text-white">
                  {stats.totalAllTime ?? 0} Mined
                </span>
                <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-amber-500/10 border border-amber-500/20 text-amber-300">
                  {stats.totalMovers ?? 0} Movers
                </span>
                <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-blue-500/10 border border-blue-500/20 text-blue-300">
                  {stats.totalIncorporations ?? 0} Formations
                </span>
                <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-purple-500/10 border border-purple-500/20 text-purple-300">
                  {stats.inOutreach ?? 0} In Outreach
                </span>
                <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
                  {stats.totalWon ?? 0} Won ({stats.conversionRate || "0%"})
                </span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-[#171717] hover:bg-[#242424] text-[#f2f2f2] px-3.5 py-1.5 rounded-lg font-medium text-xs transition-colors border border-[#2b2b2b] cursor-pointer"
            >
            ...
            <button
              ...
              disabled={syncing}
              className="flex items-center gap-1.5 bg-[#d4af37] hover:bg-[#b5952f] text-black px-4 py-1.5 rounded-lg font-semibold text-xs transition-colors shadow-sm disabled:opacity-50 cursor-pointer"
            >
        ...
        {/* Status Messages */}
        {error && (
          <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-400 rounded-lg text-xs flex items-center justify-between gap-2 shrink-0">
        ...
        {successMsg && (
          <div className="p-3 bg-green-500/10 border border-green-500/20 text-green-400 rounded-lg text-xs flex items-center justify-between gap-2 shrink-0">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 shrink-0" /> {successMsg}
            </div>
            <button onClick={() => setSuccessMsg("")} className="p-1 text-green-400 hover:text-white">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
        )}

        {/* KPI Cards (Full stats mode only) */}
        {!isHeaderCollapsed && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5 shrink-0">
            <div className="bg-[#141414] border border-[#262626] p-4 rounded-xl">
              ...
              <span className="text-[11px] text-[#737373] mt-0.5 block">Commercial entities</span>
            </div>
            ...
            <div className="bg-[#141414] border border-[#262626] p-4 rounded-xl">
              ...
              <span className="text-[11px] text-[#737373] mt-0.5 block">HR02 relocations</span>
            </div>
            ...
            <div className="bg-[#141414] border border-[#262626] p-4 rounded-xl">
              ...
              <span className="text-[11px] text-[#737373] mt-0.5 block">HR01 new setups</span>
            </div>
            ...
            <div className="bg-[#141414] border border-[#262626] p-4 rounded-xl">
              ...
              <span className="text-[11px] text-[#737373] mt-0.5 block">Contacted or Quoted</span>
            </div>
            ...
            <div className="bg-[#141414] border border-[#262626] p-4 rounded-xl">
              ...
              <span className="text-[11px] text-[#737373] mt-0.5 block">Closed contracts</span>
            </div>
          </div>
        )}

        {/* Filters and Search Bar */}
        <div className="bg-[#141414] border border-[#262626] p-3 rounded-xl flex flex-wrap items-center gap-3 shrink-0">
          <form onSubmit={handleSearchSubmit} className="relative flex-1 min-w-[260px]">
            ...
            <input
              ...
              className="w-full bg-[#1a1a1a] border border-[#262626] rounded-lg pl-9 pr-3 py-1.5 text-xs text-[#f2f2f2] placeholder-[#737373] focus:outline-none focus:border-[#d4af37]"
            />
          </form>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-[#1a1a1a] border border-[#262626] rounded-lg px-2.5 py-1.5 text-xs text-[#f2f2f2] focus:outline-none focus:border-[#d4af37]"
            >
              {CANTONS.map((c) => (
        ...
        {/* Main Workspace Area (Full Width, Trello-Style Horizontal Scrolling Kanban or Table) */}
        <div className="w-full flex-1 min-w-0">
          {loading ? (
            ...
            <div className="bg-[#141414] border border-[#262626] rounded-xl p-16 text-center flex flex-col items-center justify-center">
              ...
              <button
                onClick={() => setShowSyncModal(true)}
                className="flex items-center gap-2 bg-[#d4af37] text-black px-4 py-2 rounded-md font-semibold text-xs hover:bg-[#b5952f] transition-colors cursor-pointer"
              >
                <RefreshCw className="w-4 h-4" /> Run First Sync
              </button>
            </div>
          ) : viewMode === "kanban" ? (
            /* TRELLO-LIKE KANBAN BOARD CONTAINER: HORIZONTALLY SCROLLABLE WITH FIXED-WIDTH COLUMNS */
                ...
                {KANBAN_COLUMNS.map((col) => {
                  ...
                  return (
                    <div
                      ...
                      className={`w-[310px] min-w-[310px] max-w-[310px] shrink-0 bg-[#141414] border rounded-xl flex flex-col shadow-xl shadow-black/40 transition-all ${
                        isOver
                          ? "border-[#d4af37] bg-[#1a1a1a] ring-2 ring-[#d4af37]/20"
                          : "border-[#262626]"
                      }`}
                    >
                      {/* Column Header */}
                      <div className="flex items-center justify-between p-3.5 pb-2.5 border-b border-[#262626] bg-[#171717] rounded-t-xl">
                        <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-mono font-medium text-[#a6a6a6] bg-[#222222] px-2 py-0.5 rounded-full border border-[#2b2b2b]">
                          {colLeads.length}
                        </span>
                      </div>

                      {/* Column Cards Container (Vertical Scroll per column) */}
                      <div
                        className={`p-2.5 space-y-2.5 overflow-y-auto ${
                          isHeaderCollapsed ? "max-h-[calc(100vh-210px)]" : "max-h-[calc(100vh-320px)]"
                        }`}
                      >
                        {colLeads.length === 0 ? (
                          <div className="py-10 px-4 text-center text-xs text-[#555555] border border-dashed border-[#262626] rounded-lg">
                          ...
                          colLeads.map((lead) => {
                            ...
                            return (
                              <div
                                ...
                                className={`p-3.5 bg-[#1b1b1b] hover:bg-[#222222] border rounded-lg cursor-grab active:cursor-grabbing transition-all duration-150 space-y-2.5 text-xs relative shadow-sm ${
                                  isSelected
                                    ? "border-[#d4af37] ring-1 ring-[#d4af37]/40 bg-[#222222] shadow-md shadow-[#d4af37]/5"
                                    : "border-[#2a2a2a] hover:border-[#383838]"
                                } ${isDragging ? "opacity-35 scale-95 border-dashed border-[#d4af37]" : ""}`}
                              >
                                {/* Top Row: Score Badge + Canton + Date */}
                                <div className="flex items-center justify-between gap-2">
                                  <div className="flex items-center gap-1.5">
                                    {getPriorityBadge(lead.priorityScore)}
                                    <span className="text-[10px] font-mono text-[#a6a6a6] bg-[#262626] px-1.5 py-0.5 rounded border border-[#333333]">
                                      {lead.canton}
                                    </span>
                                  </div>
                                ...
                                <div>
                                  <h4 className="font-semibold text-[13px] text-[#f2f2f2] leading-snug break-words">
                                    {lead.companyName}
                                  </h4>
                                ...
                                {/* Relocation / Incorporation Opportunity Box */}
                                {lead.subRubric === "HR02" ? (
                                  <div className="p-2 bg-[#211a12] border border-amber-500/20 rounded-md text-[11px] space-y-1">
                                    <div className="text-amber-300 font-medium truncate flex items-center gap-1">
                                      <span>🚚</span>
                                      <span className="truncate">New: {lead.newSeat || lead.newAddress || "Office relocation"}</span>
                                    </div>
                                    {lead.oldAddress && (
                                      <div className="text-[#888888] text-[10px] truncate pl-4">
                                        Ex: {lead.oldAddress}
                                      </div>
                                    )}
                                  </div>
                                ) : (
                                  <div className="p-2 bg-[#121a24] border border-blue-500/20 rounded-md text-[11px]">
                                    <div className="text-blue-300 font-medium truncate flex items-center gap-1">
                                      <span>✨</span>
                                      <span className="truncate">New Setup: {lead.newSeat || lead.newAddress || lead.canton}</span>
                                    </div>
                                  </div>
                                )}

                                {/* Direct Outreach Quick Actions */}
                                <div className="pt-2 border-t border-[#262626] flex items-center justify-between">
                                  <div className="flex items-center gap-1.5">
                                    {lead.contactPhone ? (
                                      <a
                                        ...
                                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 text-[10px] font-medium transition-colors"
                                      ...
                                      <a
                                        ...
                                        rel="noreferrer"
                                        onClick={(e) => e.stopPropagation()}
                                        className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] text-[#a6a6a6] hover:text-[#d4af37] bg-[#222222] hover:bg-[#2a2a2a] rounded transition-colors"
                                    ...
                                    {lead.contactEmail && (
                                      <a
                                        ...
                                        onClick={(e) => e.stopPropagation()}
                                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 hover:bg-purple-500/20 text-[10px] font-medium transition-colors"
                                  ...
                                  <span className="text-[10px] text-[#737373] hover:text-[#d4af37] flex items-center gap-0.5">
                        ...
                        >
                          ...
                          <td className="p-3.5 text-right">
                            <button
                              ...
                              className="p-1.5 hover:bg-[#262626] rounded text-[#a6a6a6] hover:text-[#f2f2f2] cursor-pointer"
                            >
                ...
                <div className="p-4 border-t border-[#262626] flex items-center justify-between text-xs text-[#a6a6a6]">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={page === 1}
                      className="px-3 py-1 bg-[#1a1a1a] rounded hover:bg-[#262626] disabled:opacity-40 cursor-pointer"
                    >
                      Previous
                    </button>
                    <button
        ...
        {/* Master-Detail Slide-Over Drawer (Only opens when a lead is clicked, freeing 100% width when idle) */}
        {selectedLead && (
          <>
            {/* Backdrop */}
            <div
            ...
            {/* Slide-over Drawer */}
            <div className="fixed top-0 right-0 h-full w-full max-w-[500px] z-50 bg-[#141414] border-l border-[#262626] shadow-2xl flex flex-col animate-in slide-in-from-right duration-200">
              {/* Header */}
              <div className="p-5 border-b border-[#262626] flex items-start justify-between gap-3 bg-[#171717]">
                <div>
                  <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setSelectedLead(null)}
                  className="p-1.5 hover:bg-[#262626] rounded-md text-[#a6a6a6] hover:text-[#f2f2f2] transition-colors cursor-pointer"
                  title="Close (Esc)"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-5 text-xs">
                ...
                {/* The Relocation Opportunity Diff */}
                ...
                <div className="space-y-3 pt-2 border-t border-[#262626]">
                  ...
                  <button
                    ...
                    className="w-full bg-[#262626] hover:bg-[#333333] text-xs font-semibold py-2 rounded transition-colors text-[#f2f2f2] cursor-pointer"
                  >
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {KANBAN_COLUMNS.map((col) => (
                      <button
                        ...
                        className={`text-[11px] py-1.5 px-2 rounded font-medium transition-all cursor-pointer ${
                ...
                <div className="pt-3 border-t border-[#262626] space-y-2 pb-4">
                  {selectedLead.convertedBookingId ? (
                    ...
                    <button
                      ...
                      disabled={converting}
                      className="w-full flex items-center justify-center gap-2 bg-[#d4af37] hover:bg-[#b5952f] text-black font-semibold text-xs py-2.5 rounded-md transition-colors cursor-pointer"
                    >
            ...
            <div className="bg-[#141414] border border-[#262626] rounded-xl max-w-md w-full p-6 space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-1 text-[#737373] hover:text-[#f2f2f2] cursor-pointer"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
                  ...
                  {["ZH", "ZG", "BE", "LU", "AG", "BS", "SG", "GE"].map((code) => {
                    ...
                    return (
                      <button
                        ...
                        className={`text-xs py-1.5 rounded font-medium border transition-colors cursor-pointer ${
                          active
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#262626]">
                <button
                  type="button"
                  onClick={() => setShowSyncModal(false)}
                  className="px-4 py-2 text-xs font-medium text-[#a6a6a6] hover:text-[#f2f2f2] cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex items-center gap-2 bg-[#d4af37] hover:bg-[#b5952f] text-black px-4 py-2 rounded-md font-semibold text-xs disabled:opacity-50 cursor-pointer"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${syncing ? "animate-spin" : ""}`} />
